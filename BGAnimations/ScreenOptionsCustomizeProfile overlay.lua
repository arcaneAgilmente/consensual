...
local pad_cursor= setmetatable({}, cursor_mt)
local number_entry= new_numpad_entry{
	Name= "number_entry",
	InitCommand= cmd(diffusealpha, 0; xy, _screen.cx*1.5, _screen.cy),
	value_color= pn_to_color(PLAYER_1),
	cursor_draw= "last",
	cursor= Def.ActorFrame{
		pad_cursor:create_actors(
			"cursor", 0, 0, 1, pn_to_color(PLAYER_1), fetch_color("player.hilight"),
			{{"top", "MenuUp"}, {"bottom", "MenuDown"}, {"left", "MenuLeft"},
				{"right", "MenuRight"}}),
		MoveCommand= function(self, param)
			pad_cursor:refit(param[1], param[2], 16, 24)
		end
	},
	button_color= fetch_color("text"),
	prompt_color= fetch_color("text"),
}
...
local menu_cursor= setmetatable({}, cursor_mt)
local menu_pos= 1
local menu_start= 72
local menu_x= 32
local value_x= 300
...
local function update_menu_cursor()
	...
	menu_cursor:refit(item:GetX()-2, item:GetY(), item:GetWidth()+4, 24)
end
...
local args= {
	...
	menu_cursor:create_actors(
		"menu_cursor", 0, 0, 1, pn_to_color(PLAYER_1),
		fetch_color("player.hilight"),
		{{"top", "MenuUp"}, {"bottom", "MenuDown"}}, .5),
}
	...
	if item.get then
		...
		if item.item_type == "list" then
			...
			value_args[#value_args+1]= Def.ActorMultiVertex{
				InitCommand= function(self)
					self:SetVertices{
						{{-5, 0, 0}, fetch_color("text_other")}, {{0, -10, 0}, fetch_color("text_other")},
						{{0, 10, 0}, fetch_color("text_other")}}
